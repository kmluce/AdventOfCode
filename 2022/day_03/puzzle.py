        ...
        with open(self.fileName) as file:
            for line in file:
                ...
                if self.puzzle_part == "a":
                    compartment1 = [line[x] for x in range(0, int(len(items) / 2))]
                    compartment2 = [line[x] for x in range(int(len(items) / 2), len(items))]
                    # print("    ", compartment1)
                    # print("    ", compartment2)
                    set1 = set(compartment1)
                    set2 = set(compartment2)
                    # self.shared_items.append([value for value in compartment1 if value in compartment2])
                    for i in (set1 & set2):

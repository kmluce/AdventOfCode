...
class DropMap:
    ...
    def __init__(self, initial_x, initial_y, num_rocks=2022, debug_context=None):
        ...
        if debug_context is None:
            self.debugContext = PrintDebug(7)
        else:
    ...
    def print(self):
        # print(self.map)
        self.debugContext.print(4, "Map:")
        for i in range(self.map.shape[0] - 1, -1, -1):
            self.debugContext.print(4, ['#' if x == 1 else '.' for x in self.map[i].tolist()])
        self.debugContext.print(4, "")

    def print_rocks(self):
        for i in self.rocks:
            print(f"rock length is {len(i)}")
            print(f"rock shape is {i.shape}")
            print(f"rock is of dimension {i.ndim}")
            print(i)
            print()

    def drop_rock(self):
        self.debugContext.print(3, "printing map before padding")
        self.print()
        self.map = np.pad(self.map, [(0, 4), (0, 0)], mode='constant')
        self.debugContext.print(3, "printing map after padding")
        self.print()
        curr_rock_index = self.current_rock % len(self.rocks)
        # curr_rock_index = 1
        self.debugContext.print(3, 'curr rock is:')
        self.debugContext.print(3, self.rocks[curr_rock_index])
        rock_y = self.max_height + 3
        rock_x = 2
        self.print_rock_on_map(curr_rock_index, rock_x, rock_y)

    def drop_rocks(self):
        self.debugContext.print(1, "Starting to Drop Rocks")
        self.debugContext.increase_indent()

        for i in range(0, self.num_rocks):
            self.debugContext.print(1, f"dropping rock {i}")
            curr_rock_index = self.current_rock % len(self.rocks)
            self.debugContext.print(1, f'curr rock is {self.rocks[curr_rock_index]}')
            ...
            collision = False
            self.debugContext.print(1, f"printing rock in starting location at {rock_x},{rock_y}")
            self.print_rock_on_map(curr_rock_index, rock_x, rock_y)
            while not collision:
                curr_jet = self.jet_order[self.jet_index % len(self.jet_order)]
                self.debugContext.print(1, f"current jet is {curr_jet}")
                if curr_jet == '>':
                    if self.is_collision(curr_rock_index, rock_x + 1, rock_y):
                        self.debugContext.print(2, f"rock collided on rightward jet")
                    #            collision = True
                    # break
                    else:
                        self.debugContext.print(2, f"rock did NOT collide on rightward jet, adding 1 to x")
                        rock_x = rock_x + 1
                elif curr_jet == '<':
                    if self.is_collision(curr_rock_index, rock_x - 1, rock_y):
                        self.debugContext.print(2, f"rock collided on leftward jet")
                    #            collision = True
                    # break
                    else:
                        rock_x = rock_x - 1
                if self.is_collision(curr_rock_index, rock_x, rock_y -1):
                    self.debugContext.print(2, f"rock collided on drop")
                    collision = True
                    break
                elif self.is_rest(curr_rock_index, rock_x, rock_y):
                    self.debugContext.print(2, f"rock came to rest")
                    collision = True
                    break
                else:
                    rock_y = rock_y - 1
            ...
            self.jet_index = self.jet_index + 1
            self.debugContext.print(1, f"adding rock to map at {rock_x}, {rock_y}")
            self.add_rock_to_map(curr_rock_index, rock_x, rock_y)
            self.debugContext.print(1, f"printing map after rock")
            self.debugContext.print(1, "")
            if rock_y + rock_height > self.max_height:
                self.max_height = rock_y + rock_height
            # self.max_height = self.max_height + rock_height
            self.debugContext.print(1, f"max height is now {self.max_height}")
    ...
    def print_rock_on_map(self, rock, x, y):
        ...
        for i in range(tmp_map.shape[0] - 1, -1, -1):
            #self.debugContext.print(4, tmp_map[i])
            self.debugContext.print(4, ['#' if x == 1 else '@' if x == 2 else '.' for x in tmp_map[i].tolist()])
        self.debugContext.print(4,"")
    ...
    def is_collision(self, rock, x, y):
        ...
        if y < 0:
            self.debugContext.print(3, f"rock collides because y < 0")
            return True
        elif x < 0:
            self.debugContext.print(3, f"rock collides because x < 0")
            return True
        elif x + rock_width - 1 > 6:
            self.debugContext.print(3, f"rock collides because x + rock is out of bounds")
            return True
        for test_y in range(0, rock_height):
            for test_x in range(0, rock_width):
                if self.rocks[rock].ndim == 1:

...
class PlaneSeating():
    ...
    def valid_seat(self, row, col):
        ...
        if col < 0:
            return False
        if row >= len(self.seatMap):
            return False
        if col >= len(self.seatMap[row]):
            return False
        return True
    ...
    def check_surrounding_seats(self, col, row):
        ...
        for surround_row in range(row - 1, row + 2, 1):
            for surround_col in range(col - 1, col + 2, 1):
                ...
                if self.valid_seat(surround_row, surround_col):
                    if not (surround_col == col and surround_row == row):
                        if self.seatMap[surround_row][surround_col] == 'L':
                            ...
                            open_col = surround_col
                            open_row = surround_row
                            while self.valid_seat(open_row, open_col) and self.seatMap[open_row][open_col] == '.':
                                open_col += col_differential
                                open_row += row_differential
                            if self.valid_seat(open_row,open_col):
                                if self.seatMap[open_row][open_col] == 'L':
                                    seatInventory[0] += 1
                                if self.seatMap[open_row][open_col] == '#':
                                    seatInventory[1] += 1
